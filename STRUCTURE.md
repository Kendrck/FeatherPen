...
FeatherPen/src/database/sql_init.sql | local_user表结构升级，新增bind_email、bind_phone字段，创建全局唯一索引防重复绑定
...
FeatherPen/member_config.json | 全等级账号配置统一修正UID为6位数字，密码标准化为passwd，统一标注账号输入规范，固化Lv0~Lv9积分999999999
...
FeatherPen/requirements.txt | 移除PyQt6、electron相关依赖
...
FeatherPen/docs/FeatherPen（羽笔）V1.0.0 开发文档.docx | 同步更新账号校验、数据库唯一索引、前后端双层校验全套规范
